...
class Task(object):
    ...
    def part4(self):
        ...
        # fields: name, day, time (NO header requested)
    ...
    def part6(self):
        ...
        fin = open('hours.csv', 'r', newline='')
        reader = csv.reader(fin)

        f = open('part6.txt', 'w')
        for row in reader:
            f.write(str(row))   # no newline, exactly like console list format
        f.close()
        fin.close()
    ...
    def part7(self):
        ...
        for row in reader:
            for cell in row:
                f.write(cell)    # no spaces, no newlines
        f.close()
        fin.close()
